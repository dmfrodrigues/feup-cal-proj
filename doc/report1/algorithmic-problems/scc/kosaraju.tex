...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item \label{itm:Agood} If $SCC(u)=SCC(v)$ their order is arbitrary in $L$, since we can consider either paths $u \leadsto v$ or $v \leadsto u$ to order them ($A \cap P \cap D$).
        \item \label{itm:Abad} The above case is the only valid way of having both $u \leadsto v$ and $u \in A$; otherwise, we would have $u \leadsto v$, $u \not \leadsto v$ and $u \in A$ ($A \cap P \cap \overline{D}$), which is obviously a violation of the fact nodes in $L$ are in pre-order, since $u \leadsto v$ and $u \not \leadsto v$ is undisputable evidence that $u$ must be before $v$ in $L$ (\sout{$A \cap P \cap \overline{D}$}).
    \end{enumerate}
\end{enumerate}
